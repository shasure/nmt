...
def train(hparams, scope=None, target_session=""):
    ...
    # TODO(zsy): change training session; not use Supervisor to save summaries and ckpt; only use Supervisor to init ops, prepare session and load ckpt
    is_chief = (hparams.job_name and hparams.task_index == 0)
    sv = tf.train.Supervisor(graph=train_model.graph, is_chief=is_chief, summary_op=None, save_model_secs=0,
                             init_op=train_model.model.init_op,
                             local_init_op=train_model.model.chief_local_init_op if is_chief else train_model.model.local_init_op,
                             logdir=out_dir,
                             # global_step=global_step,
                             ready_for_local_init_op=train_model.model.ready_for_local_init_op)
    train_sess = sv.prepare_or_wait_for_session(master=target_session, config=config_proto)
    if is_chief:
        # Chief worker will start the chief queue runner and call the init op.
        train_sess.run(train_model.model.sync_init_op)
        sv.start_queue_runners(train_sess, [train_model.model.chief_queue_runner])
    ...
    # TODO(zsy): Since Supervisor will finalize graph, graph can not change anymore after create Supervisor
